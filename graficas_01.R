...
df <- read.csv("incendios_en_regiones.csv", header = TRUE) %>% 
  ...
  mutate(date_acq = ymd(acq_date),time_acq = hm(str_c(str_sub(acq_time,1,-3),":",str_sub(acq_time,-2,-1)))) %>% 
...
red_time <- df %>% group_by(date_acq,DESECON1) %>% 
  ...
  mutate(year=year(date_acq),moy=month(date_acq),doy=yday(date_acq))
