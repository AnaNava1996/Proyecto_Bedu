...
incendios<-read.csv("./Data_Sets/incendios_con_ecoregiones_y_tiposdesuelo.csv") %>% 
  filter(DESECON1!="",GRUPO_FINA!="") %>% 
  mutate(date_acq = ymd(acq_date),time_acq = hm(acq_time),year=year(date_acq),moy=month(date_acq))

by_ecoregion <- df %>% group_by(date_acq,doy,DESECON1) %>% 
  # contamos los puntos q ocurrieron en misma fecha misma ecorregion
  summarise(c0unt = n()) %>% 
  mutate(doy=yday(date_acq))

by_tiposuelo <- df %>% group_by(date_acq,doy,GRUPO_FINA) %>% 
  # contamos los puntos q ocurrieron en misma fecha mismo tipo/uso suelo
  summarise(c0unt = n()) %>% 
  mutate(doy=yday(date_acq))
...
=======
incendios <-
  na.omit(read.csv("./Data_Sets/incendios_con_ecoregiones_y_tiposdesuelo.csv"))
...
graph <- tabItem(tabName = "graph",
                 fluidRow(
                   ...
                   selectInput(
                     "z",
                     "Selecciona la variable del grid",
                     choices = c("cyl", "vs", "gear", "carb")
                   ),
                   box(plotOutput(
                     "output_plot", height = 300, width = 460
                   ))
                 ))

data.table <- tabItem(tabName = "data_table",
                      fluidRow(titlePanel(h3("Data Table")),
                               dataTableOutput ("data_table")))

img <- tabItem(tabName = "img",
               fluidRow(
                 titlePanel(h3(
                   "Imágen de calor para la correlación de las variables"
                 )),
                 img(src = "ecorregiones.png", align = "center",
                     width = 750),
                 img(src = "conabio_vegetacion.png", align = "center",
...
ui <-
  fluidPage(dashboardPage(
    skin = "purple",
    dashboardHeader(title = "Proyecto, análisis de incendios en México",
                    titleWidth = 450),
    dashboardSidebar(
      width = 350,
      sidebarMenu(
...
>>>>>>> 7443fb05e96a98eb7227ff879e94cc380ebfaa09
        )
      )
    ),
...
<<<<<<< HEAD
  output$interannual_ecoreg_plot <- renderPlot({ 
    ggplot(filter(by_ecoregion,DESECON1==input$rowName1),aes(x = `date_acq`,y=`c0unt`)) +
      geom_smooth() +
  ...
  output$intrannual_ecoreg_plot <- renderPlot({ 
    ggplot(filter(by_ecoregion,DESECON1==input$rowName1),aes(x = `doy`,y=`c0unt`)) +
      geom_smooth() +
      #scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
  ...
  output$interannual_tiposuelo_plot <- renderPlot({ 
    ggplot(filter(by_tiposuelo,GRUPO_FINA==input$rowName2),aes(x = `date_acq`,y=`c0unt`)) +
      geom_smooth() +
      scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
      coord_cartesian(ylim=c(0, 30)) +
      labs(
  ...
  output$intrannual_tiposuelo_plot <- renderPlot({ 
    ggplot(filter(by_tiposuelo,GRUPO_FINA==input$rowName2),aes(x = `doy`,y=`c0unt`)) +
      geom_smooth() +
      #scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
      #coord_cartesian(ylim=c(0, 30)) +
      labs(
...
>>>>>>> 7443fb05e96a98eb7227ff879e94cc380ebfaa09
  ...
  output$map <- renderGoogle_map({
    ...
    entre_fechas <-
      function(fecha) {
        incendios[incendios$acq_date == fecha, ]
      }
